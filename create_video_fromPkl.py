...
for file in file_names:
    ...
    for folder in next(os.walk(parent_dir))[1]: 
        ...
        print(f"{folder}-{file}")
        # create image from pkl
        plt.clf()
        fig, ax = plt.subplots()
        # concatenate dataframes
        pkl_file = pathlib.Path(parent_dir).joinpath(folder, "voltage", name + '.pkl')
        concatenated = pd.read_pickle(pkl_file)
        concatenated.T.plot(ylim=(0.99*vmin,1.01*vmax), title=f'{name}', legend=False)
        fig.tight_layout()
        output_img = pathlib.Path(parent_dir).joinpath(folder, "voltage", file) 
